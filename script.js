/*
 * Yathome
 * Copyright (c) 2023-2024 Yatharth
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    const defaultApiKey = "ec0792cfe9644f60ab9112211243011"; // Default Weather API key
...
document.getElementById(
  "second"
).style.transform = `rotate(${cumulativeSecondRotation}deg)`;
document.getElementById(
  "minute"
).style.transform = `rotate(${cumulativeMinuteRotation}deg)`;
document.getElementById(
  "hour"
).style.transform = `rotate(${cumulativeHourRotation}deg)`;
...
function updateDate() {
  if (clocktype === "analog") {
    ...
    const dateDisplay = {
      ...
      it: `${dayName.substring(0, 3)} ${dayOfMonth} ${monthName.substring(
        0,
        3
      )}`,
      ja: `${dayName.substring(0, 1)}, ${monthName}${dayOfMonth}`,
      ko: `${dayName.substring(0, 1)}, ${monthName} ${dayOfMonth}일`,
      pt: `${dayName.substring(0, 3)}, ${dayOfMonth} ${monthName.substring(
        0,
        3
      )}`,
      ru: `${dayName.substring(0, 2)}, ${dayOfMonth} ${monthName.substring(
        0,
        4
      )}.`,
      es: `${dayName.substring(0, 3)}, ${dayOfMonth} ${monthName.substring(
        0,
        3
      )}`,
      ...
      default: `${dayName.substring(0, 3)}, ${monthName.substring(
        0,
        3
      )} ${localizedDayOfMonth}`,
    };
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const PLACEHOLDER_SHORTCUT_URL =
    "https://github.com/Yatharth/materialYouNewTab";
